...
const char * const BITCOIN_CONF_FILENAME = "nefelicoin.conf";
...
#else
    const char* pszModule = "nefelicoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Nefelicoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Nefelicoin";
#else
    // Unix-like
    return pathRet / ".nefelicoin";
#endif
#endif
}
