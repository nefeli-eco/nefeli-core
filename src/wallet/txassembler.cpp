    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional NFLC outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes NFLC and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::NFLC_TO_NFLC) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a NFLC-to-NFLC transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::NFLC_ONLY;
        ...
        // If NFLC-to-NFLC fails, create a peg-out transaction
