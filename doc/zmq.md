...
The Nefelicoin Core daemon can be configured to act as a trusted "border
router", implementing the nefelicoin wire protocol and relay, making
...
The ZeroMQ feature in Nefelicoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building nefelicoind:
...
For instance:
    ...
    $ nefelicoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/nefelicoind.tx.raw \
...
These options can also be provided in nefelicoin.conf.
...
From the perspective of nefelicoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into nefelicoind directly. Furthermore, no information is
...
using. Nefelicoind appends an up-counting sequence number to each
